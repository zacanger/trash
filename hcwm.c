...
void grabkeys() {
  ...
  for (i = 0; i < TABLENGTH(keys); ++i) {
    if ((code = XKeysymToKeycode(dis, keys[i].keysym))) {
      XGrabKey(dis, code, keys[i].mod, root, True, GrabModeAsync,
               GrabModeAsync);
    }
  }
}
...
void move_down() {
  Window tmp;
  if (current == NULL || current->next == NULL || current->win == head->win ||
      current->prev == NULL) {
    return;
  }
  tmp = current->win;
...
void tile() {
  ...
  if (head != NULL && head->next == NULL) {
    XMoveResizeWindow(dis, head->win, gaps, gaps, sw - (3 * gaps),
                      sh - (gaps * 2));
  }
  /* side stack */
    ...
    case 0:
      /* Master window */
      XMoveResizeWindow(dis, head->win, gaps, gaps, master_size - (2 * gaps),
                        sh - (2 * gaps));
    ...
    case 1:
      for (c = head; c; c = c->next) {
        XMoveResizeWindow(dis, c->win, gaps, gaps, sw - (2 * gaps),
                          sh - (2 * gaps));
      }
      break;
    }
  ...
  /*bottom stack*/
    ...
    case 0:
      /* Master window */
      XMoveResizeWindow(dis, head->win, gaps, gaps, sw - (2 * gaps),
                        master_size - (2 * gaps));
